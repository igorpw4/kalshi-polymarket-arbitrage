...
Pior: a divergência não é aleatória. O sinal do delta entre as réguas determina
*geometricamente* qual tipo de divergência pode ocorrer, e 79% dos deltas são
negativos. Isso torna um dos dois lados da operação **negativo em valor
esperado** (−$0,05/contrato), enquanto o outro vira uma aposta direcional
disfarçada. O bot não distinguia os dois.
...
**Valor esperado por lado da operação** (12% de divergência, viés de 79%):

| Posição | EV por contrato |
|---|---|
| arb #1 (YES Kalshi + NO Poly) | **−$0,0496** |
| arb #2 (NO Kalshi + YES Poly) | +$0,0896 |

O bot executou a arb #1 em 36 das 82 entradas, sem nunca distinguir os casos.
